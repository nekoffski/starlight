...
void EulerCamera::processInput(const float speed) {
    if (core::WindowManager::get().isKeyPressed(SL_KEY_W)) m_pitch -= speed;
    if (core::WindowManager::get().isKeyPressed(SL_KEY_S)) m_pitch += speed;
    if (core::WindowManager::get().isKeyPressed(SL_KEY_D)) m_yaw -= speed;
    if (core::WindowManager::get().isKeyPressed(SL_KEY_A)) m_yaw += speed;

    if (core::WindowManager::get().isKeyPressed(SL_KEY_1)) m_radius += speed / 10.0f;
    if (core::WindowManager::get().isKeyPressed(SL_KEY_2)) m_radius -= speed / 10.0f;
}
