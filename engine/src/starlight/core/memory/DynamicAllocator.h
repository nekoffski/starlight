...
class DynamicAllocator {
   ...
   public:
    explicit DynamicAllocator(uint64_t size)
        : m_totalSize(size), m_freeList(size), m_memory(size, 0), m_memoryAlias(m_memory.data()) {
    ...
    void free(void* block, uint64_t size) {
        ...
        if (block < m_memoryAlias || block > m_memoryAlias + m_totalSize) {
            LOG_ERROR(
                "Trying to free block {} outside of allocator range {} - {}", block, m_memoryAlias,
                m_memoryAlias + m_totalSize
            );
            return;
        }
    ...
    uint64_t spaceLeft() { return m_freeList.spaceLeft(); }
