...
Buffer::Buffer(const Context* context, const Device* device, const Buffer::Properties& props)
    : m_context(context)
    , m_device(device)
...
VkMemoryRequirements Buffer::getMemoryRequirements(VkBuffer buffer) const {
    ...
    vkGetBufferMemoryRequirements(m_device->getLogicalDevice(), buffer, &memoryRequirements);
...
void Buffer::bind(uint64_t offset) {
    VK_ASSERT(vkBindBufferMemory(m_device->getLogicalDevice(), m_handle, m_memory, offset));
}
...
bool Buffer::resize(uint64_t size, VkQueue queue, VkCommandPool pool) {
    ...
    auto memoryRequirements = getMemoryRequirements(buffer);  // TODO: pass VkBuffer here!
...
void Buffer::copyTo(
    ...
    CommandBuffer tempCommandBuffer{m_device, pool, CommandBuffer::Severity::primary};
    tempCommandBuffer.createAndBeginSingleUse();

    vkCmdCopyBuffer(tempCommandBuffer.getHandle(), m_handle, destination, 1, &copyRegion);
