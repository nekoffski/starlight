...
void VKUIShader::createGlobalDescriptorSetLayout() {
...
void VKUIShader::createGlobalDescriptorPool(int swapchainImageCount) {
...
void VKUIShader::createLocalDescriptorPool() {
    ...
    objectPoolInfo.maxSets       = maxUICount;
...
void VKUIShader::createLocalDescriptorSetLayout() {
...
void VKUIShader::createShaderStages() {
    ...
    m_stages.emplace_back(
      ...
      VKShaderStage::Properties{
        fs.readFile(
          "/home/nek0/kapik/projects/starlight/assets/shaders/UIShader.vert.spv"
        ),
        ShaderStage::vertex }
    );
    ...
    m_stages.emplace_back(
      ...
      VKShaderStage::Properties{
        fs.readFile(
          "/home/nek0/kapik/projects/starlight/assets/shaders/UIShader.frag.spv"
        ),
        ShaderStage::fragment }
    );
}

void VKUIShader::use(VKCommandBuffer& buffer) {
    m_pipeline->bind(buffer, VK_PIPELINE_BIND_POINT_GRAPHICS);
}

void VKUIShader::createPipeline(
  const Size2u32& framebuffferSize, VKRenderPass& renderPass
    ...
    uint64_t sizes[attribute_count] = { sizeof(Vec2f), sizeof(Vec2f) };
...
VKUIShader::~VKUIShader() {
    ...
    LOG_TRACE("Destroying VKUIShader instance");
...
VkDescriptorSetLayout VKUIShader::getObjectDescriptorSetLayout() const {
    return m_objectDescriptorSetLayout;
}

void VKUIShader::setModel(VkCommandBuffer commandBuffer, const Mat4f& model) {
    vkCmdPushConstants(
...
void VKUIShader::updateGlobalWorldState(
...
void VKUIShader::createUniformBuffer() {
    ...
    props.size = sizeof(InstanceUBO) * maxUICount;
...
VkDescriptorSetLayout VKUIShader::getGlobalDescriptorSetLayout() const {
    return m_globalDescriptorSetLayout;
}

const std::vector<VKShaderStage>& VKUIShader::getStages() const { return m_stages; }

VKUIShader::GlobalUBO& VKUIShader::getGlobalUBO() { return m_globalUBO; }

void VKUIShader::acquireResources(Material& material) {  // TODO: free list
...
void VKUIShader::releaseResources(Material& material) {
    auto& instanceState = m_instanceStates[material.internalId];
