...
class FirstPersonCamera : public Camera {
   public:
    ...
    void processInput(const float speed) {
        if (core::WindowManager::get().isKeyPressed(NOVA_KEY_W)) m_position -= speed * m_front;
        if (core::WindowManager::get().isKeyPressed(NOVA_KEY_S)) m_position += speed * m_front;
        if (core::WindowManager::get().isKeyPressed(NOVA_KEY_D)) m_position += speed * m_right;
        if (core::WindowManager::get().isKeyPressed(NOVA_KEY_A)) m_position -= speed * m_right;
    }
