...
inline float toDegrees(float rad) { return glm::degrees(rad); }
...
inline Mat4 scale(Vec3 scale) { return glm::scale(glm::mat4(1.0f), scale); }

inline Mat4 scale(float s) { return scale(Vec3{s}); }

inline float toRadians(float deg) { return glm::radians(deg); }

inline Vec3 toRadians(const Vec3& vec) {
    return Vec3{toRadians(vec.x), toRadians(vec.y), toRadians(vec.z)};
}

inline Vec3 normalize(const Vec3& vec) { return glm::normalize(vec); }
