...
class AssetContainer2 {
   public:
    ...
    T* getByName(const std::string& name) { return m_nameToAsset.at(name).get(); }

    T* getById(const std::string& id) { return m_nameToAsset.at(m_idToName.at(id)).get(); }
