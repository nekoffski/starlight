...
OpenGlShader::OpenGlShader(
    const std::string& vertexPath, const std::string& fragmentPath, const std::string& geomPath
)
...
void OpenGlShader::setUniform(const std::string& name, unsigned int value) {
    glProgramUniform1ui(
        m_shaderProgram, glGetUniformLocation(m_shaderProgram, name.c_str()), value
    );
}

void OpenGlShader::setUniform(const std::string& name, math::Mat3 value) {
    glProgramUniformMatrix3fv(
        m_shaderProgram, glGetUniformLocation(m_shaderProgram, name.c_str()), 1, GL_FALSE,
        math::valuePtr(value)
    );
}
...
void OpenGlShader::setUniform(const std::string& name, math::Vec4 value) {
    glProgramUniform4f(
        m_shaderProgram, glGetUniformLocation(m_shaderProgram, name.c_str()), value.x, value.y,
        value.z, value.w
    );
}
