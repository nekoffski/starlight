...
GuiHelper::FontWrapper&& GuiHelper::FontWrapper::mergeWith(
    ...
    ImGui::GetIO().Fonts->AddFontFromFileTTF(
        path.c_str(), m_size, &config, m_parentMergedFontsRanges.back().data()
    );

    return std::move(*this);
}

GuiHelper::FontWrapper GuiHelper::addFont(
    const std::string& name, const std::string& path, const int size
) {
