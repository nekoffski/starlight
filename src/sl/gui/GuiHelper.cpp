...
GuiHelper::FontWrapper::FontWrapper(const int size, MergedFontsRanges& parentMergedFontsRanges)
    : m_size(size)
    , m_parentMergedFontsRanges(parentMergedFontsRanges) {
}

GuiHelper::FontWrapper&& GuiHelper::FontWrapper::mergeWith(
    ...
    ImGui::GetIO().Fonts->AddFontFromFileTTF(path.c_str(), m_size,
        &config, m_parentMergedFontsRanges.back().data());

    return std::move(*this);
}

GuiHelper::FontWrapper GuiHelper::addFont(const std::string& name, const std::string& path, const int size) {
    ASSERT(not m_fonts.contains(name), "Font with name " + name + " is already loaded");

    m_fonts[name] = Font {
        .handle = ImGui::GetIO().Fonts->AddFontFromFileTTF(path.c_str(), size)
    };

    return FontWrapper {
        size, m_fonts.at(name).mergedFontsRanges
    };
}
